function [Vnet,conc,v] = solve_ode(expanded_model,t_interval,y0,kinetic_param)
...
try
  ...
  eval(strcat('options=odeset(''Jacobian'',@(t,y)jacobFun_',expanded_model.model_name,...
      '(t,y,kinetic_param),''Events'',@(t,y)event_function(t,y,a,kinetic_param,S_f_b));')); 
  ...
  %---------- Compute Vnet ----------------
   Vnet = zeros(size(expanded_model.unexpanded_model.rxn,1),time_point_num);
%   
  for j=1:size(expanded_model.unexpanded_model.rxn,1)
    for t=1:time_point_num
